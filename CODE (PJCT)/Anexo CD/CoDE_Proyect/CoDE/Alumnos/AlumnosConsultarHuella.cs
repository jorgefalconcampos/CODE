...
{
    public partial class AlumnosConsultarHuella : Form
    {
        public AlumnosConsultarHuella()
        { InitializeComponent(); }

        private void AlumnosConsultarHuella_Load(object sender, EventArgs e)
        {
        ...
        {
            try
            { string AA = "Buen día, esta ventana le ayuda a conocer la huella digital de un alumno en específico..."
        ...
        {
            Procesos.Opciones.ID = txtAlumnoConsultar.Text;
        ...
        {
            AlumnosConsultarPreview VerHuella = new AlumnosConsultarPreview();
            VerHuella.ID_Alumno = txtAlumnoConsultar.Text;
        ...
        private void btnLimpiar_Click(object sender, EventArgs e)
        {
            ...
            lblEncontrado.Visible = false; lblEncontrado.Text = ""; txtAlumnoConsultar.Text = "";
        }

        private void btnBuscar_Click(object sender, EventArgs e)
        { Contraer(); Consulta(); BuscarAlumno(txtAlumnoConsultar.Text); }
        ...
        private void Consulta()
        {
            try
            {
                ...
                AdaptadorDeDatos.SelectCommand.Parameters["@Boleta"].Value = int.Parse(txtAlumnoConsultar.Text);
                AdaptadorDeDatos.Fill(AjusteDatos, "tb_Alumnos");

                if (AjusteDatos.Tables["tb_Alumnos"].Rows.Count != 0)
                {
                    byte[] ImgBuffer = (byte[])AjusteDatos.Tables["tb_Alumnos"].Rows[0]["Foto"];
                    MemoryStream ms = new MemoryStream(ImgBuffer);
                    ImgAlumno.Visible = true; 
                    ImgAlumno.Image = Image.FromStream(ms);
                    /////////////////////////////////////////////////////////
                    byte[] ImgBuffer2 = (byte[])AjusteDatos.Tables["tb_Alumnos"].Rows[0]["HuellaAlumno"];
                ...
                } else { MessageBox.Show("No se pudo encontrar el alumno o uno o más de sus registros...", "Error", MessageBoxButtons.OK, MessageBoxIcon.Exclamation); }
            } catch { MessageBox.Show("No se puede encontrar el alumno o uno o más de sus registros. Compruebe si el alumno existe o si fue registrado correctamente.", 
                "Error", MessageBoxButtons.OK, MessageBoxIcon.Exclamation); }
        }

        private void BuscarAlumno(string AlumnoBoleta)
        {
            Visor.DataSource = Procesos.ConsAlum_1(AlumnoBoleta);
            if (Visor.Rows.Count > 0)
            {
                ...
                lblEncontrado.ForeColor = Color.ForestGreen; this.Size = new Size(875, 353);
                btnCerrar.Location = new Point(128, 260); btnLimpiar.Location = new Point(350, 260);
                btnBuscar.Location = new Point(572, 260); GrupoMasDatos.Visible = true; btnVerMas.Visible = true;
            }
        }
        ...
        private void AlumnosConsultarHuella_FormClosing(object sender, FormClosingEventArgs e)
